...
baseThemes.forEach(base => {
    ...
    writeFileSync(
        join(process.cwd(), base.file.replace('.json', '').replace('-color-theme', '-bold-color-theme.json')), 
        JSON.stringify(boldTheme, null, 2)
    );
    ...
    writeFileSync(
        join(process.cwd(), base.file.replace('.json', '').replace('-color-theme', '-italic-color-theme.json')), 
        JSON.stringify(italicTheme, null, 2)
    );
});
