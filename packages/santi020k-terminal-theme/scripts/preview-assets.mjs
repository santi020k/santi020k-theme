...
export const renderPreviewSvg = palette => {
  ...
  const segments = promptSegments.map((segment, index) => {
    const width = widths.at(index)
    const fill = colors.get(segment.key)
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="760" height="190" viewBox="0 0 760 190"><rect width="760" height="190" rx="18" fill="${colors.get('canvas')}"/><g font-family="ui-monospace, SFMono-Regular, Menlo, monospace" font-size="16" font-weight="600">${segments}<text x="38" y="154" fill="${colors.get('text')}">❯ pnpm run focus</text></g></svg>\n`
}
