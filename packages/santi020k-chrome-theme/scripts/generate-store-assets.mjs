#!/usr/bin/env node
/**
 * Generates Chrome Web Store assets for the dark theme (default) variant.
 *
 * The unsuffixed files in store/assets are the dark listing assets.
 * This script writes them from the same HTML-rendered approach used by
 * generate-store-assets-light.mjs so both variants stay in sync.
 */

import { spawnSync } from 'child_process';
import { mkdirSync, rmSync, writeFileSync } from 'fs';
import { tmpdir } from 'os';
import { dirname, join } from 'path';
import { fileURLToPath, pathToFileURL } from 'url';

const themePackageRoot = dirname(fileURLToPath(import.meta.resolve('@santi020k/theme/package.json')));
const outDir = join(themePackageRoot, 'assets', 'chrome', 'store');
const tmpDir = join(tmpdir(), 'santi020k-dark-store-assets');
const chromeBin = process.env.CHROME_BIN || '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';

const html = ({ width, height, body }) => `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=${width},height=${height},initial-scale=1">
  <style>
    :root {
      --ink: #dfdde3;
      --muted: #a19da8;
      --soft: #0d0a15;
      --panel: #15121f;
      --panel-2: #231d30;
      --line: #231d30;
      --accent: #752df0;
      --accent-2: #945df4;
      --accent-3: #b48df7;
      --deeper: #09070f;
      --deeper-2: #06050b;
      color: var(--ink);
      font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
    }
    * { box-sizing: border-box; }
    html, body { width: ${width}px; height: ${height}px; margin: 0; overflow: hidden; background: var(--soft); }
    body { letter-spacing: 0; }
    .asset { width: ${width}px; height: ${height}px; position: relative; overflow: hidden; background: linear-gradient(135deg, #0d0a15 0%, #110c1d 56%, #15121f 100%); }
    .subtle-grid {
      position: absolute;
      inset: 0;
      background-image:
        linear-gradient(rgba(117, 45, 240, .08) 1px, transparent 1px),
        linear-gradient(90deg, rgba(117, 45, 240, .08) 1px, transparent 1px);
      background-size: 48px 48px;
      mask-image: linear-gradient(135deg, transparent 0%, #000 18%, #000 72%, transparent 100%);
    }
    .brand { font-weight: 760; line-height: .94; letter-spacing: 0; }
    .kicker { color: var(--accent-2); font-weight: 720; letter-spacing: 0; text-transform: uppercase; }
    .copy { color: var(--muted); font-weight: 540; line-height: 1.35; }
    .swatches { display: flex; gap: 10px; }
    .swatch { width: 38px; height: 38px; border-radius: 8px; border: 1px solid rgba(223, 221, 227, .12); }
    .chrome { position: absolute; overflow: hidden; border: 1px solid rgba(223, 221, 227, .10); box-shadow: 0 28px 90px rgba(0, 0, 0, .5); background: var(--soft); }
    .chrome-bar { height: 88px; background: var(--panel); border-bottom: 1px solid var(--line); }
    .tabs { height: 38px; display: flex; align-items: end; gap: 4px; padding: 8px 14px 0; background: var(--soft); }
    .tab { width: 184px; height: 30px; border-radius: 9px 9px 0 0; padding: 8px 14px; font-size: 12px; color: var(--muted); background: var(--soft); border: 1px solid transparent; position: relative; }
    .tab.active { color: var(--ink); background: var(--panel); border-color: var(--line); border-bottom-color: var(--panel); }
    .tab.active:before { content: ""; position: absolute; left: 18px; right: 18px; top: 0; height: 3px; border-radius: 3px; background: var(--accent); }
    .toolbar { height: 50px; display: flex; align-items: center; gap: 12px; padding: 9px 16px; }
    .dot { width: 13px; height: 13px; border-radius: 50%; background: var(--line); }
    .omnibox { flex: 1; height: 32px; display: flex; align-items: center; padding: 0 16px; border-radius: 999px; background: var(--soft); border: 1px solid var(--line); color: var(--muted); font-size: 13px; }
    .star { width: 18px; height: 18px; border-radius: 50%; border: 3px solid var(--accent-2); }
    .bookmark-row { height: 30px; display: flex; align-items: center; gap: 18px; padding: 0 20px; font-size: 12px; color: var(--muted); border-bottom: 1px solid rgba(35, 29, 48, .9); }
    .bookmark:before { content: ""; display: inline-block; width: 8px; height: 8px; border-radius: 2px; background: var(--accent-3); margin-right: 7px; }
    .page { position: absolute; left: 0; right: 0; top: 118px; bottom: 0; overflow: hidden; background: var(--soft); }
    .search { width: 560px; height: 48px; margin: 82px auto 34px; border-radius: 999px; border: 1px solid var(--line); background: var(--panel); box-shadow: 0 10px 28px rgba(0, 0, 0, .3); color: var(--muted); display: flex; align-items: center; padding: 0 22px; font-size: 15px; }
    .ntp-mark { position: absolute; right: 132px; top: 176px; width: 360px; height: 360px; border: 3px dashed var(--line); display: grid; place-items: center; color: var(--accent-2); font-size: 104px; font-weight: 800; line-height: 1; }
    .tiles { display: grid; grid-template-columns: repeat(4, 104px); gap: 18px; justify-content: center; }
    .tile { height: 92px; border: 1px solid var(--line); background: var(--panel); border-radius: 8px; padding: 14px; box-shadow: 0 10px 22px rgba(0, 0, 0, .3); }
    .tile-icon { width: 28px; height: 28px; border-radius: 7px; background: var(--panel-2); margin-bottom: 14px; }
    .tile-line { height: 8px; border-radius: 4px; background: var(--line); }
    .side-panel { position: absolute; left: 72px; top: 176px; width: 310px; }
    .side-title { font-size: 42px; line-height: 1; margin: 0 0 14px; color: var(--ink); font-weight: 780; }
    .side-copy { margin: 0; color: var(--muted); font-size: 16px; line-height: 1.45; }
    .incognito .chrome-bar { background: var(--deeper); border-bottom-color: #1a1729; }
    .incognito .tabs { background: var(--deeper-2); }
    .incognito .tab { background: var(--deeper-2); color: #6e687a; }
    .incognito .tab.active { background: var(--deeper); color: #b6b2bd; border-color: #1a1729; border-bottom-color: var(--deeper); }
    .incognito .toolbar .dot { background: #2a2436; }
    .incognito .omnibox { background: var(--deeper-2); border-color: #1a1729; color: #b6b2bd; }
    .incognito .bookmark-row { background: var(--deeper); color: #6e687a; border-bottom-color: #1a1729; }
  </style>
</head>
<body>${body}</body>
</html>`;

const swatches = (colors, size = 38) => colors.map((color) => `<span class="swatch" style="width:${size}px;height:${size}px;background:${color}"></span>`).join('');

const tile = index => {
  const colors = ['#752df0', '#945df4', '#b48df7', '#231d30'];

  return `<div class="tile"><div class="tile-icon" style="background:${colors[index % colors.length]}"></div><div class="tile-line"></div></div>`;
};

const chromeTop = (isIncognito = false) => `<div class="chrome-bar">
    <div class="tabs">
      <div class="tab active">${isIncognito ? 'New Incognito Tab' : 'Santi020k Theme'}</div>
      <div class="tab">VS Code Marketplace</div>
      <div class="tab">GitHub</div>
    </div>
    <div class="toolbar">
      <span class="dot"></span><span class="dot"></span><span class="dot"></span>
      <div class="omnibox">chrome.santi020k.com</div>
      <span class="star"></span>
    </div>
  </div>`;

const promoTile = () => `<main class="asset">
  <div class="subtle-grid"></div>
  <section style="position:absolute;left:34px;top:34px;width:250px">
    <div class="kicker" style="font-size:12px;margin-bottom:18px">Dark Chrome Theme</div>
    <div class="brand" style="font-size:46px;color:var(--ink)">Santi020k</div>
    <p class="copy" style="font-size:15px;margin:16px 0 24px">Deep violet chrome with luminous accent hues.</p>
    <div class="swatches">
      ${swatches(['#0d0a15', '#15121f', '#231d30', '#752df0', '#dfdde3'])}
    </div>
  </section>
  <div class="ntp-mark" style="right:24px;top:58px;width:112px;height:112px;font-size:36px;border-width:2px">&gt;_</div>
</main>`;

const marqueeBanner = () => `<main class="asset">
  <div class="subtle-grid"></div>
  <section style="position:absolute;left:82px;top:82px;width:470px">
    <div class="kicker" style="font-size:16px;margin-bottom:24px">Dark Chrome Theme</div>
    <div class="brand" style="font-size:94px;color:var(--ink)">Santi020k</div>
    <p class="copy" style="font-size:24px;margin:28px 0 38px">A deep browser frame tuned to the Santi020k palette.</p>
    <div class="swatches" style="gap:14px">
      ${swatches(['#0d0a15', '#15121f', '#231d30', '#752df0', '#945df4', '#b48df7', '#dfdde3'], 54)}
    </div>
  </section>
  <div class="chrome" style="right:72px;top:92px;width:650px;height:376px;border-radius:10px">
    ${chromeTop()}
    <div class="page">
      <div class="side-panel" style="left:48px;top:62px;width:220px">
        <h1 class="side-title" style="font-size:31px">Dark mode, sharp signal.</h1>
        <p class="side-copy" style="font-size:14px">Deep violet surfaces, legible text, and bright focus.</p>
      </div>
      <div class="ntp-mark" style="right:56px;top:56px;width:190px;height:190px;font-size:56px">&gt;_</div>
    </div>
  </div>
</main>`;

const browserScreenshot = ({ mode }) => {
  const isIncognito = mode === 'incognito';

  return `<main class="asset ${isIncognito ? 'incognito' : ''}">
  <div class="chrome" style="inset:0;border:0;box-shadow:none;border-radius:0">
    ${chromeTop(isIncognito)}
    ${mode === 'ntp' ? '' : '<div class="bookmark-row"><span class="bookmark">Docs</span><span class="bookmark">Marketplace</span><span class="bookmark">Santi020k</span><span class="bookmark">Design</span></div>'}
    <div class="page" style="top:${mode === 'ntp' ? '88px' : '118px'}">
      <div class="subtle-grid"></div>
      <div class="side-panel">
        <h1 class="side-title">${isIncognito ? 'Private, polished.' : 'Santi020k Dark'}</h1>
        <p class="side-copy">${isIncognito ? 'Incognito deepens the frame while keeping text crisp and readable.' : 'Deep violet surfaces with bright accents for tabs, links, and focus states.'}</p>
      </div>
      <div class="search">chrome.santi020k.com</div>
      <div class="ntp-mark">&gt;_</div>
      <div class="tiles" style="grid-template-columns:repeat(3, 104px);margin-top:320px;transform:translateX(-72px)">${Array.from({ length: 3 }, (_, i) => tile(i)).join('')}</div>
    </div>
  </div>
</main>`;
};

const pages = [
  { name: 'promo-tile.png', width: 440, height: 280, body: promoTile() },
  { name: 'marquee-banner.png', width: 1400, height: 560, body: marqueeBanner() },
  { name: 'screenshot-main.png', width: 1280, height: 800, body: browserScreenshot({ mode: 'main' }) },
  { name: 'screenshot-ntp.png', width: 1280, height: 800, body: browserScreenshot({ mode: 'ntp' }) },
  { name: 'screenshot-incognito.png', width: 1280, height: 800, body: browserScreenshot({ mode: 'incognito' }) },
];

mkdirSync(outDir, { recursive: true });

rmSync(tmpDir, { recursive: true, force: true });

mkdirSync(tmpDir, { recursive: true });

for (const page of pages) {
  const htmlPath = join(tmpDir, `${page.name}.html`);
  const outPath = join(outDir, page.name);

  writeFileSync(htmlPath, html(page), 'utf8');

  const result = spawnSync(chromeBin, [
    '--headless=new',
    '--disable-gpu',
    '--disable-dev-shm-usage',
    '--hide-scrollbars',
    '--no-first-run',
    `--window-size=${page.width},${page.height}`,
    `--screenshot=${outPath}`,
    pathToFileURL(htmlPath).href,
  ], { stdio: 'pipe' });

  if (result.status !== 0) {
    process.stderr.write(result.stderr.toString());

    process.stderr.write(result.stdout.toString());

    throw new Error(`Failed to render ${page.name}`);
  }

  console.log(`Generated store/assets/${page.name}`);
}
